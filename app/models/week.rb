...
#      errors.add :week, "Invalid week." if !valid_weeks.flatten.member?(self.from.cweek) or !valid_weeks.flatten.member?(self.to.cweek)
      ...
      if Hash[*valid_weeks.flatten][self.id] and Hash[*valid_weeks.flatten][self.id] != (self.from.cweek && self.to.cweek)
        errors.add :unique_week, "Each column should have unique weeks."
      end
    end
  end
end
